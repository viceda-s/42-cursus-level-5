...
bool Config::parse() {
	...
	default_config.max_body_size = 10485760; // 10MB
	...
	root_location.methods.push_back("HEAD");
	root_location.methods.push_back("PUT");
	...
	upload_location.root = "./www";
	upload_location.upload_path = "./uploads";
	upload_location.methods.push_back("GET");
	upload_location.methods.push_back("POST");
	upload_location.methods.push_back("DELETE");
	upload_location.methods.push_back("HEAD");
	upload_location.methods.push_back("PUT");

	default_config.locations.push_back(upload_location);
...
void Config::_parseServerBlock(const std::string& block, ServerConfig& config) {
	// Track character position in block to correctly find each location's { }
	size_t block_search_pos = 0;
	...
	for (size_t i = 0; i < lines.size(); ++i)
	{
		...
		else if (line.find("max_body_size") == 0 || line.find("client_max_body_size") == 0)
		{
			std::vector<std::string> tokens = _split(line, ' ');
			if (tokens.size() >= 2)
			{
		...
		{
			// Find the opening brace for this location block starting from where
			// we last finished (not from line index `i` which is wrong)
			size_t start = block.find("{", block_search_pos);
			if (start == std::string::npos) break;
			size_t end = _findClosingBrace(block, start);
			if (end == std::string::npos) break;
			std::string loc_block = block.substr(start + 1, end - start - 1);
			// Advance past this location block for the next iteration
			block_search_pos = end + 1;
...
void Config::_parseLocationBlock(const std::string& block, LocationConfig& location) {
	...
	for (size_t i = 0; i < lines.size(); ++i)
	{
		...
		else if (line.find("allowed_methods") == 0 || line.find("methods") == 0)
		{
			std::vector<std::string> tokens = _split(line, ' ');
